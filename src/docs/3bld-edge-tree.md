...
出典: ネット上の解説記事2本(固有名は意図的に省略):
1. **カタログ記事** — UFバッファのエッジ3-style全440手順を、実戦採用手順+所見つきで網羅したもの(2025年)
2. **入門記事** — 「組み合わせ → 系統」の判定ルールを9カテゴリで体系化したもの(2020年加筆版)
...
- コミュテータ表記(カタログ記事は海外の有名手順表に由来する記法):
  - `[A, B]` = A B A' B'
